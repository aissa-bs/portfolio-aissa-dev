...
function App() {
  ...
  return (
    <div
      className="relative w-screen h-screen overflow-hidden text-zinc-900 dark:text-zinc-100 selection:bg-blue-500/30 transition-colors duration-500"
      ref={containerRef}
    >
      ...
      <div
        ref={sidebarConstraintsRef}
        className="absolute top-4 left-4 right-4 bottom-4 pointer-events-none z-0 hidden md:block"
      />
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0">
        ...
        >
          ...
          <div className="fixed top-6 left-6 z-[2000] md:hidden">
            <button
              onClick={() => setMobileMenuOpen(true)}
              className="p-3 rounded-full bg-white/20 dark:bg-black/20 backdrop-blur-md border border-black/5 dark:border-white/10 text-zinc-800 dark:text-zinc-100 hover:bg-white/30 dark:hover:bg-white/10 transition-all shadow-lg active:scale-95"
      ...
      <Sidebar
        ...
        constraintsRef={sidebarConstraintsRef}
      />

      {/* Bottom Right Bar: Socials + Clock */}
      <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-[2000] hidden md:flex flex-col-reverse md:flex-row items-end md:items-center gap-3">
        <SocialBar />
        <ClockBar />
      </div>
        ...
        {mobileMenuOpen && (
          <div className="fixed inset-0 z-[100] bg-zinc-50/95 dark:bg-zinc-900/95 backdrop-blur-xl flex flex-col items-center justify-center space-y-8 md:hidden transition-colors duration-300">
            <button
              ...
              className="absolute top-6 right-6 p-2 bg-black/5 dark:bg-white/10 rounded-full text-zinc-900 dark:text-white"
            >
              <Menu />
            </button>
            {NAV_ITEMS.map((item) => (
